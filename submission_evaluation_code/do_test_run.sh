...
DOCKER_ENV_ARGS=(
  ...
  -e "JUDGE_DTYPE=${JUDGE_DTYPE}"
  -e "JUDGE_MAX_NEW_TOKENS=${JUDGE_MAX_NEW_TOKENS}"
...
echo "=+= Doing a forward pass (USE_GPUS=${USE_GPUS}, CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}, JUDGE_DEVICE=${JUDGE_DEVICE}, JUDGE_DTYPE=${JUDGE_DTYPE}, JUDGE_MAX_NEW_TOKENS=${JUDGE_MAX_NEW_TOKENS}, JUDGE_MODEL_PATH=${JUDGE_MODEL_PATH}, INCLUDE_PER_CASE_RESULTS=${INCLUDE_PER_CASE_RESULTS}, INCLUDE_EVALUATION_DETAILS=${INCLUDE_EVALUATION_DETAILS})"
